...
public class ConfigModule extends AbstractModule {
    ...
    private Connection getLocalConnection() {
        /* Методи-провайдери -- розширення засобів інжкції на випадки,
           коли требя більше контролю за об'єктами
           - у служби параметризовані конструктори і треба ними керувати
           - треба більше контролю за життєвим циклом служб
           Методи "орієнтуються" на тип опвернення (тут - Connection), тобто
           постачають об'єкти в точки інжекції заданого типу.
           За необхідністі кількох однотипних провайдерів - іменування
         */
...
/* Inversion of Control (IoC) - інверсфя управління
Шснує декілька близьких термінів
DI - Dependency Injection
DIP - Dependency Inversion Principle (лікра D у SOLID)
IoC - Inversion of Control

Інверсія управління (чим?) жмттєвим циклом об'єктів - виділення певного
...
переходимо до інструкції
 obj = IoC/getInstance(Class) які можуть вжити раніше створені об'єкти / або створювати нові

DI - інжекція залежностей - спосіб (засіб) передачі залежних об'єктів у
інші об'єкти. Розрізняють інжекцію у поря, у конструктор, у методи
...
Це значно спрощує перехід до нових реалізацій залежностей
    DependencyNew : IService {} залищається сумісною з Class { IService _dependency; }
...
Spring - потужний комплекс, не лише призначений для IoC (бажано розглянути самостійно)
...
2. Створюємо клас-конфігуратор (ConfigModule) як нащадок ФиіефсеЬщвгду (com.google.inject.AbstractModule)
    перезначаємо у ньому метод-конфігуратор (configure())
